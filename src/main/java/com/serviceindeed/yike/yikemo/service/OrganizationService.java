...
public class OrganizationService {
     ...
     * @return
     */
    public Map<String, Object> queryPageOrganizationList(Organization organization, HttpPages httpPages) {

        if (YiKeMoHelper.getInstance().isPaging(httpPages)) { //分页
            Page<Organization> page = PageHelper.offsetPage(httpPages.getStart(), httpPages.getLength()).doSelectPage(() -> organizationMapper.queryOrganizationList(organization));
            return YiKeMoHelper.getInstance().getSuccessPageQueryJson(httpPages.getDraw() + 1, page);
        } else {
            List<Organization> organizationList = organizationMapper.queryOrganizationList(organization);
